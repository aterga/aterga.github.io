...
function text(d) {
    if (d.text) {
        ...
        return d.text.map((paragraph) => paragraph).join('\n')
    }
    return "This section is under construction"
}

function text_length(d) {
    if (d.text) {
        return d.text.reduce((accum, paragraph) => accum + paragraph.length, 0)     
    }
    return d.value
}
...
function render(group, root) {
    ...
    node.append("clipPath")
        .attr("id", (d,i) => (d.clipUid = "clip" + i).id)
      .append("use")
